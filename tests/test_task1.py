...
def test_add_student():
    ...
    assert students.students[0].grades == [5, 4, 5, 5, 4]


def test_sort_students_by_group():
    students = Students()
    students.add_student("Иванов И.И.", 102, [5, 4, 5, 5, 4])
    students.add_student("Петров П.П.", 101, [3, 4, 4, 5, 3])
    assert students.students[0].group_number == 101
...
def test_select_students():
    students = Students()
    students.add_student("Иванов И.И.", 101, [5, 4, 5, 5, 4])
    students.add_student("Петров П.П.", 102, [3, 4, 3, 4, 3])
...
def test_unknown_command_error():
    with pytest.raises(UnknownCommandError) as excinfo:
        raise UnknownCommandError("test_command")
    assert str(excinfo.value) == "test_command -> Unknown command"
